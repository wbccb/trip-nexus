...
> 本质就是将目前完成的固定工作流用 Agent节点的形式进行编排+状态存储（后续可恢复）+可视化处理

- Orchestrator：基于 LangGraph 的 StateGraph 编排顺序链路（Planner → Checker → Optimizer → Map/RAG），并通过 graph.stream 输出流式更新。[orchestrator.py](file:///Users/wcbbcc/my-code/my_github/TripNexus/src/agent/orchestrator.py)
- 事件总线与快照：EventBus 统一记录 node_start/node_end/interrupt/error/tool_call/tool_result/update；SnapshotStore 按步骤落快照，支持从最新快照恢复。[event_bus.py](file:///Users/wcbbcc/my-code/my_github/TripNexus/src/agent/event_bus.py)
- HITL 与恢复：支持 pause_at（planner/checker）触发暂停，Streamlit 面板“继续执行”从最近快照恢复
- Streamlit 调试面板：侧边栏展示事件流与快照面板，可调链路开关、预算上限、行程密度、室内优先、工具偏好。[ui_manager.py](file:///Users/wcbbcc/my-code/my_github/TripNexus/src/frontend/ui_manager.py)

核心代码逻辑说明（最小闭环）：
- thread_id：一次 Agent 执行链路的“会话指针”，事件过滤、快照归属、恢复都以它为主键（UI 在 session_state 中持久化保存）
- StateGraph 编排：用节点（planner/checker/optimizer/map_rag）与边描述顺序链路；通过 conditional_edges 根据 agent_config（enable_checker/enable_optimizer/enable_rag）决定是否跳过某些步骤
- 流式事件：执行时使用 graph.stream(...) 逐 chunk 输出更新；每个 chunk 原样写入 EventBus(kind=update)，便于 UI 做时间线/回放
- 节点包装器：所有节点函数都由 _wrap_node 包装，自动完成 node_start/node_end 事件打点、耗时统计、异常捕获与 retry_limit 限次重试
- 快照落盘（内存）：每个节点结束后构造 snapshot（step/payload/logs/duration_ms/state 子集）写入 SnapshotStore；恢复时读取 latest 快照的 state 并重进图执行，由于节点函数具备“已有结果直接复用”的幂等判断，因此能快速跳过已完成步骤

节点与状态（当前实现）：
- 节点数量：4 个（planner、checker、optimizer、map_rag）
- 全局执行状态（state.status）：
  - running：执行中（默认）
  - paused：命中 pause_at 进入中断（HITL）
  - failed：节点执行失败且超过 retry_limit（会记录 state.error）
  - cancelled：外部标记取消（当前实现保留 cancelled 字段与事件类型，UI 未接入显式取消按钮）
- 节点级状态（以事件流为准）：
  - pending：尚未开始（UI 侧可推断：没有该节点的 node_start）
  - running：收到 node_start
  - done：收到 node_end 且未进入 failed/paused
  - paused：收到 interrupt（同时 state.status=paused）
  - failed：收到 error 且最终 node_end 的 updates.status=failed（同时 state.status=failed）
  - skipped：链路开关关闭导致节点不会进入（如 enable_checker=False 时 checker skipped）
...
- Agent Map/RAG 节点透传 evidence 到 UI：[_map_rag](file:///Users/wcbbcc/my-code/my_github/TripNexus/src/agent/orchestrator.py#L528-L553)
